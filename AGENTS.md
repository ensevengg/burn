...
| # | Decision | Rule |
|---|----------|------|
| D1 | Data flow | Machines **push** on a schedule (default 10-min cron + run-on-wake catch-up) **and** optionally run `burn-report daemon` (resident, ~30s poll of `sync_requests`) so a phone refresh reaches online machines within seconds. The scheduled push is the reliability floor; the daemon is the eager path. The phone never talks to machines directly. Tailscale direct-pull is v2 — now in progress on `feat/tailscale-live-pull` per [docs/adr/0001](docs/adr/0001-tailscale-direct-pull.md): an additive freshness layer, never a replacement for the push/mirror path. |
...
- Demo seed is already batched via `execAsync`; cloud event sync now also batches bound inserts (32 rows per statement).
...
- **Mobile performance + quota refresh (2026-09-06)**: cached timezone formatters, cooperative/cancellable aggregation, shared bounded event reads, focused query gating, virtualized Explore lists and SQL-limited environment/client-scoped sessions. Cloud event writes use 32-row bound batches; independent quota pulls publish immediately, with writes still serialized. Phone refresh follows up for two minutes and pulls on foreground/each active minute; reset cancels old responses. Reporter `push` and daemon now fetch events + quotas independently, and daemon polls cannot overlap. See `docs/mobile-performance-audit.md` and `scripts/diagnostics/mobile-perf.ts`; Android timing validation is still pending.
- **Post-audit smoothing pass (2026-09-06)**: event queries no longer heartbeat (quotas/machines only — per-minute full re-reads removed), `keepPreviousData` on window/metric queries, event-cache eviction lives inside the writers post-commit (`resetDb`, demo seed, `removeEnvironmentLocal`), pull status split into `useSyncStatus` (backfill progress is a notice, not an error; refresh spinner covers the first pull only), all touch targets are `Pressable` (Segmented included), `computeStreaks` walks active ordinals, contribution cells memoized, dashboard token headline/shares include reasoning tokens, and migration `0005_delta_revision_index.sql` adds `usage_events (revision, event_id)` for delta pulls. See `docs/mobile-performance-audit.md` "Second pass"; Android timing validation still pending.
- **Tailscale live-pull (2026-09-07, branch `feat/tailscale-live-pull`)**: machines running `daemon` (or `serve`) expose a read-only tailnet-bound HTTP endpoint serving the same rows the next push would send; the phone probes on foreground/refresh gestures only and merges into the mirror with **revision 0** and `ON CONFLICT ... WHERE usage_events.revision = 0` (live can never overwrite server rows), event ids via `liveEventId()` = the server's sha256 recipe (verified against node:crypto). Watermark never moves on the live path. Migration `0006_live_endpoint.sql` adds the advertisement (set/clear/preserve semantics). Live quota merge deferred (cloud quota pull replaces the table wholesale — needs freshness-aware upserts first). E2E smoke against the real exporter passed (234 rows, ids stable, revisions=0).
- **Direct mode decision (2026-09-07, ADR [docs/adr/0002](docs/adr/0002-direct-mode-supabase-demoted.md), not yet implemented)**: Supabase is demoted to backup — it stays deployed and accumulating, but direct (Tailscale) mode becomes the primary path: machines-as-backend via `pullDirect`, per-machine time cursors, manual add-machine UX, quotas merged per-env on day one. Cloud mode stays fully functional; the two collide harmlessly on shared event ids. Phase-1 design is pinned in the ADR; implementation is a separate branch.
