...
- Known deferrals (post-demo iteration): `burn-events` Rust exporter + upstream `tokscale events --jsonl` PR (unblocks real `push`), victory-native/Skia chart pass, EAS build profiles, reporter service-install helpers (`install-service`).
...
- Dev commands: `bun install`, `bun run typecheck`, `bun run test` (root). Reporter: `bun run --cwd apps/reporter dev -- <cmd>`. Mobile: `bun run --cwd apps/mobile start` (Expo Go for the demo).
